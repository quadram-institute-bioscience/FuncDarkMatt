...
def main(input, output, verbose):
    ...
    Example usage:
    python parse_enzrxn-dat.py -i enzrxns.dat -o output.csv -f concatenated
    """
    
    if verbose:
        click.echo(f"Reading enzrxns.dat file: {input}")
    
    try:
        # Parse the enzrxns file
        enzrxns = parse_enzrxns_file(input)
        ...
        click.echo(f"Successfully wrote output to: {output}")
